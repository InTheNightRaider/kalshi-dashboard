...
export default function SettingsModal({ kalshiKeySet, githubConnected, githubUsername, githubRepo, onClose, onSaved }: Props) {
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-end" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="h-full w-full max-w-md bg-[#111318] border-l border-[#252c3a] shadow-2xl flex flex-col overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#252c3a]">
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-3">
        <div className="w-7 h-7 bg-[#f5c842]/10 rounded-lg flex items-center justify-center">
        ...
        <div>
          <p className="text-white font-medium text-sm">Kalshi API Key</p>
      ...
      <p className="text-gray-500 text-xs mb-3">
        ...
        </a>. Replaces your current key.
      </p>
      <form onSubmit={handleSave} className="flex gap-2">
        <input
          type="password"
          placeholder="New Kalshi API key..."
          value={key}
          onChange={e => setKey(e.target.value)}
          className="input flex-1 font-mono text-xs"
        />
        <button type="submit" disabled={saving || !key.trim()} className="btn-primary shrink-0 text-sm">
          {saving ? 'Saving...' : 'Update'}
        </button>
      </form>
      {error   && <p className="text-[#ff4d6d] text-xs mt-2">{error}</p>}
      {success && <p className="text-[#00d17a] text-xs mt-2">✓ Kalshi API key updated</p>}
    </div>
  )
}
...
function GitHubSection({ connected, username, repo, onSaved }: { connected: boolean; username: string | null; repo: string | null; onSaved: () => void }) {
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-3">
        <div className="w-7 h-7 bg-[#4f8ef7]/10 rounded-lg flex items-center justify-center">
      ...
      {repos.length === 0 ? (
        <>
          <p className="text-gray-500 text-xs mb-3">
            Enter a new PAT to reconnect or switch accounts. Needs <code className="text-gray-400">repo + workflow</code> scopes.
          </p>
