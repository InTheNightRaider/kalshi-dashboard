  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="relative h-2 bg-[#1a1f2e] rounded-full overflow-hidden">
        ...
        <div className="absolute top-0 bottom-0 w-px bg-[#f5c842]/60"
    ...
    <div className={`rounded-lg border p-3 space-y-2.5 ${borderClass}`}>
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="text-white font-mono font-semibold text-sm mt-0.5">Strike {shortTicker}</div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-xs">
        ...
        <div>
          ...
          <div className={`font-mono font-semibold ${yesMid >= 0.90 && yesMid <= 0.96 ? 'text-[#00d17a]' : 'text-gray-300'}`}>
            {(yesMid * 100).toFixed(1)}c
          </div>
        </div>
        <div>
          <div className="text-gray-500 text-[10px] mb-0.5">NO Mid</div>
          <div className={`font-mono font-semibold ${noMid >= 0.90 && noMid <= 0.96 ? 'text-[#00d17a]' : 'text-gray-300'}`}>
            {(noMid * 100).toFixed(1)}c
          </div>
        </div>
        <div>
          <div className="text-gray-500 text-[10px] mb-0.5">Volume</div>
          <div className="font-mono font-semibold text-gray-300">{volume.toLocaleString()}</div>
        </div>
        <div>
          <div className="text-gray-500 text-[10px] mb-0.5">Min In</div>
          <div className={`font-mono font-semibold ${minutesIn >= 7 && minutesIn <= 12 ? 'text-[#00d17a]' : 'text-gray-400'}`}>
            {minutesIn.toFixed(1)}m
          </div>
        </div>
        <div>
          <div className="text-gray-500 text-[10px] mb-0.5">Min Left</div>
          <div className={`font-mono font-semibold ${minutesLeft >= 3 && minutesLeft <= 8 ? 'text-[#00d17a]' : 'text-gray-400'}`}>
...
export default function ScanSnapshot() {
  ...
  const qualifies = data !== null && data.confluenceDir !== null && data.confluenceScore >= 4
  ...
  return (
    <div className="card">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <span className="relative flex h-2 w-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#00d17a] opacity-75" />
            <span className="relative inline-flex rounded-full h-2 w-2 bg-[#00d17a]" />
          </span>
          <h3 className="font-semibold text-white text-sm">Live Scanner</h3>
          <span className="text-gray-600 text-xs font-normal">Mode 4 · 7-TF Confluence</span>
        </div>
        <span className="text-gray-600 text-xs">
          {loading ? 'Fetching...' : data ? `${age}s ago` : 'No data'}
        </span>
      </div>

      {loading && (
        <div className="py-6 text-center text-gray-600 text-sm animate-pulse">Computing signals...</div>
      )}

      {!loading && data && (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-[#0d1117] border border-[#1a1f2e] rounded-lg px-3 py-2.5">
              <div className="text-gray-500 text-[10px] mb-0.5">BTC / USD</div>
              <div className="text-white font-mono font-semibold text-sm">
                ${data.btcPrice ? Math.round(data.btcPrice).toLocaleString() : '-'}
              </div>
            </div>
            <div className="bg-[#0d1117] border border-[#1a1f2e] rounded-lg px-3 py-2.5">
              <div className="text-gray-500 text-[10px] mb-0.5">RSI 1m</div>
              <div className={`font-mono font-semibold text-sm ${
                data.rsi1m !== null ? data.rsi1m > 60 ? 'text-[#ff4d6d]' : data.rsi1m < 40 ? 'text-[#00d17a]' : 'text-gray-300' : 'text-gray-600'
              }`}>{data.rsi1m !== null ? data.rsi1m.toFixed(1) : '-'}</div>
            </div>
            <div className="bg-[#0d1117] border border-[#1a1f2e] rounded-lg px-3 py-2.5">
              <div className="text-gray-500 text-[10px] mb-0.5">RSI 5m</div>
              <div className={`font-mono font-semibold text-sm ${
                data.rsi5m !== null ? data.rsi5m > 60 ? 'text-[#ff4d6d]' : data.rsi5m < 40 ? 'text-[#00d17a]' : 'text-gray-300' : 'text-gray-600'
              }`}>{data.rsi5m !== null ? data.rsi5m.toFixed(1) : '-'}</div>
            </div>
          </div>

          <div>
            <div className="text-gray-500 text-[10px] mb-2">Timeframe signals</div>
            <div className="flex flex-wrap gap-1.5">
              {TFS.map(tf => <SignalPill key={tf} tf={tf} signal={data.signals[tf] ?? null} />)}
            </div>
          </div>

          <ConfluenceBar score={data.confluenceScore} total={data.totalTFs} dir={data.confluenceDir} />

          {data.contract && <ContractCard contract={data.contract} />}

          <div className={`rounded-lg px-3 py-2.5 border text-xs font-medium flex items-center gap-2 ${
            qualifies
              ? 'bg-[#00d17a]/10 border-[#00d17a]/30 text-[#00d17a]'
              : 'bg-[#1a1f2e] border-[#252c3a] text-gray-500'
          }`}>
            <span className="text-base">{qualifies ? 'OK' : '...'}</span>
            {qualifies
              ? `Confluence met — Mode 4 will enter next qualifying contract (${data.confluenceDir} bias, 0.90-0.96 mid filter active)`
              : `Waiting for ${Math.max(0, 4 - data.confluenceScore)} more TF${4 - data.confluenceScore !== 1 ? 's' : ''} to align · entry window: 3-8 min remaining`
            }
          </div>
        </div>
      )}
      ...
      {!loading && !data && (
        <div className="py-4 text-center text-gray-600 text-sm">Could not reach Binance API</div>
      )}
    </div>
  )
}
