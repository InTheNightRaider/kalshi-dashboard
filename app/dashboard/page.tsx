...
export default function DashboardPage() {
  ...
  // ── Sync bot status badge with real GitHub Actions state ─────────────────────
  const fetchBotStatus = useCallback(async () => {
    // Don't overwrite a user-initiated transition (starting/stopping)
    setBotStatus(prev => {
      if (prev === 'starting' || prev === 'stopping') return prev
      return prev // will be set async below
    })
    try {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      fetchAll()
      // Re-sync with GitHub Actions after a few seconds
